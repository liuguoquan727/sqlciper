...
public abstract class AbstractDao<T, K> {
  ...
  /**
   * Detaches an entity from the identity scope (session). Subsequent query results won't return
   * this object.
   */
  ...
  /**
   * Detaches all entities (of type T) from the identity scope (session). Subsequent query results
   * won't return any
  ...
  /**
   * Inserts the given entities in the database using a transaction. The given entities will become
   * tracked if the PK
   * is set.
   *
   * @param entities The entities to insert.
   * @param setPrimaryKey if true, the PKs of the given will be set after the insert; pass false to
   * improve
   * performance.
   */
  ...
  /**
   * Inserts or replaces the given entities in the database using a transaction. The given entities
   * will become
  ...
  /**
   * Insert an entity into the table associated with a concrete DAO <b>without</b> setting key
   * property.
   * <p>
   * Warning: This may be faster, but the entity should not be used anymore. The entity also won't
   * be attached to
   * identity scope.
   *
   * @return row ID of newly inserted entity
  ...
  private long executeInsert(T entity, SQLiteStatement stmt, boolean setKeyAndAttach) {
    long rowId;
  ...
  private long insertInsideTx(T entity, SQLiteStatement stmt) {
    synchronized (stmt) {
      bindValues(stmt, entity);
  ...
  /**
   * "Saves" an entity to the database: depending on the existence of the key property, it will be
   * inserted
   * (key is null) or updated (key is not null).
   * <p>
   * This is similar to {@link #insertOrReplace(Object)}, but may be more efficient, because if a
   * key is present,
  ...
  /**
   * Unlock identityScope during cursor.moveToNext() when it is about to fill the window (needs a db
   * connection):
  ...
  /**
   * Creates a repeatable {@link Query} object based on the given raw SQL where you can pass any
   * WHERE clause and
   * arguments.
   */
  ...
  /**
   * Deletes an entity with the given PK from the database. Currently, only single value PK entities
   * are supported.
   */
  ...
  public void refresh(T entity) {
    ...
    try {
      boolean available = cursor.moveToFirst();
      if (!available) {
        throw new DaoException("Entity does not exist in the database anymore: "
            + entity.getClass()
            + " with key "
            + key);
      } else if (!cursor.isLast()) {
  ...
  /**
   * Sub classes with relations additionally set the DaoMaster here. Must be called before the
  ...
  /**
   * See {@link #getKey(Object)}, but guarantees that the returned key is never null (throws if
   * null).
   */
  ...
  /**
   * The returned RxDao is a special DAO that let's you interact with Rx Observables using RX's IO
   * scheduler for
   * subscribeOn.
   *
   * @see #rxPlain()
  ...
  /**
   * Reads the key from the current position of the given cursor, or returns null if there's no
   * single-value key.
   */
  ...
  /**
   * Binds the entity's values to the statement. Make sure to synchronize the statement outside of
   * the method.
   */

  /**
   * Binds the entity's values to the statement. Make sure to synchronize the enclosing
   * DatabaseStatement outside
   * of the method.
   */
  protected abstract void bindValues(SQLiteStatement stmt, T entity);

  /**
   * Updates the entity's key if possible (only for Long PKs currently). This method must always
   * return the entity's
