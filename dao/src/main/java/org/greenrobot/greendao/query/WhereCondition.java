...
/**
 * Internal interface to model WHERE conditions used in queries. Use the {@link Property} objects in
 * the DAO classes to
...
public interface WhereCondition {
  ...
  abstract class AbstractCondition implements WhereCondition {
    ...
    @Override public void appendValuesTo(List<Object> valuesTarget) {
      if (hasSingleValue) {
  ...
  class PropertyCondition extends AbstractCondition {
      ...
      if (type == Date.class) {
        if (value instanceof Date) {
          ...
          throw new DaoException(
              "Illegal date value: expected java.util.Date or Long for value " + value);
        }
        ...
        if (value instanceof Boolean) {
          ...
          if (intValue != 0 && intValue != 1) {
            throw new DaoException(
                "Illegal boolean value: numbers must be 0 or 1, but was " + value);
          }
    ...
    @Override public void appendTo(StringBuilder builder, String tableAlias) {
