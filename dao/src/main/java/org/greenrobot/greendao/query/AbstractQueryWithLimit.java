...
abstract class AbstractQueryWithLimit<T> extends AbstractQuery<T> {
  ...
  protected AbstractQueryWithLimit(AbstractDao<T, ?> dao, String sql, String[] initialValues,
  ...
  /**
   * Sets the parameter (0 based) using the position in which it was added during building the
   * query. Note: all
  ...
  /**
   * Sets the offset for results returned by this Query. {@link org.greenrobot.greendao.query.QueryBuilder#offset(int)}
   * must
   * have been called on
