...
/**
 * Builds custom entity queries using constraints and parameters and without SQL (QueryBuilder
 * creates SQL for you). To
 * acquire an QueryBuilder, use {@link AbstractDao#queryBuilder()} or {@link
 * AbstractDaoSession#queryBuilder(Class)}.
 * Entity properties are referenced by Fields in the "Properties" inner class of the generated
 * DAOs.
 * This approach
 * allows compile time checks and prevents typo errors occuring at build time.<br/>
 * <br/>
 * Example: Query for all users with the first name "Joe" ordered by their last name. (The class
 * Properties is an inner
...
public class QueryBuilder<T> {
   ...
   * (see {@link #orderAsc(Property...)} and {@link #orderDesc(Property...)}). This uses "COLLATE
   * LOCALIZED", which
  ...
  /**
   * Customizes the ordering of strings used by {@link #orderAsc(Property...)} and {@link
   * #orderDesc(Property...)}.
  ...
  /**
   * Creates a WhereCondition by combining the given conditions using OR. The returned
   * WhereCondition must be used
   ...
   * used to match the
   * given destinationProperty of the given destinationEntity. Note that destination entity of the
   * given join is used
   * as the source for the new join to add. In this way, it is possible to compose complex "join of
   * joins" across
  ...
  private <J> Join<T, J> addJoin(String sourceTablePrefix, Property sourceProperty,
      AbstractDao<J, ?> destinationDao, Property destinationProperty) {
    String joinTablePrefix = "J" + (joins.size() + 1);
    Join<T, J> join =
        new Join<T, J>(sourceTablePrefix, sourceProperty, destinationDao, destinationProperty,
            joinTablePrefix);
    joins.add(join);
    return join;
  }
  ...
  /**
   * Adds the given raw SQL string to the ORDER BY section. Do not use this for standard properties:
   * orderAsc and
  ...
  /**
   * Sets the offset for query results in combination with {@link #limit(int)}. The first {@code
   * offset} results are
   * skipped and the total number of results will be limited by {@code limit}. You cannot use offset
   * without limit.
   */
  ...
  /**
   * Builds a reusable query object (Query objects can be executed more efficiently than creating a
   * QueryBuilder for
   * each execution.
   */
   ...
   * (Query objects can be executed more efficiently than creating a QueryBuilder for each
   * execution.
   */
  ...
  /**
   * Shorthand for {@link QueryBuilder#build() build()}.{@link Query#list() list()}; see {@link
   * Query#list()} for
   * details. To execute a query more than once, you should build the query and keep the {@link
   * Query} object for
   * efficiency reasons.
   */
  public List<T> list() {
   ...
   * {@link Query#listLazy()} for details. To execute a query more than once, you should build the
   * query and keep the
  ...
  /**
   * Shorthand for {@link QueryBuilder#build() build()}.{@link Query#listLazyUncached()
   * listLazyUncached()}; see
   * {@link Query#listLazyUncached()} for details. To execute a query more than once, you should
   * build the query and
  ...
  /**
   * Shorthand for {@link QueryBuilder#build() build()}.{@link Query#listIterator() listIterator()};
   * see
   * {@link Query#listIterator()} for details. To execute a query more than once, you should build
   * the query and keep
  ...
  /**
   * Shorthand for {@link QueryBuilder#build() build()}.{@link Query#unique() unique()}; see {@link
   * Query#unique()}
   * for details. To execute a query more than once, you should build the query and keep the {@link
   * Query} object for
   * efficiency reasons.
   */
  public T unique() {
  ...
  /**
   * Shorthand for {@link QueryBuilder#build() build()}.{@link Query#uniqueOrThrow()
   * uniqueOrThrow()}; see
   * {@link Query#uniqueOrThrow()} for details. To execute a query more than once, you should build
   * the query and
   * keep
   * the {@link Query} object for efficiency reasons.
   */
  ...
  /**
   * Shorthand for {@link QueryBuilder#buildCount() buildCount()}.{@link CountQuery#count()
   * count()}; see
   * {@link CountQuery#count()} for details. To execute a query more than once, you should build the
   * query and keep
