...
public class Query<T> extends AbstractQueryWithLimit<T> {
  private final static class QueryData<T2> extends AbstractQueryData<T2, Query<T2>> {
    ...
    QueryData(AbstractDao<T2, ?> dao, String sql, String[] initialValues, int limitPosition,
        int offsetPosition) {
    ...
    @Override protected Query<T2> createQuery() {
      return new Query<T2>(this, dao, sql, initialValues.clone(), limitPosition, offsetPosition);
    }
  }

  /** For internal use by greenDAO only. */
  public static <T2> Query<T2> internalCreate(AbstractDao<T2, ?> dao, String sql,
  ...
  private Query(QueryData<T> queryData, AbstractDao<T, ?> dao, String sql, String[] initialValues,
      int limitPosition, int offsetPosition) {
  ...
  /**
   * Executes the query and returns the result as a list that lazy loads the entities on first
   * access. Entities are
   * cached, so accessing the same entity more than once will not result in loading an entity from
   * the underlying
  ...
  /**
   * Executes the query and returns the result as a list that lazy loads the entities on every
   * access (uncached).
  ...
  /**
   * Executes the query and returns the result as a list iterator; make sure to close it to close
   * the underlying
  ...
  @Override public Query<T> setParameter(int index, Object parameter) {
    return (Query<T>) super.setParameter(index, parameter);
  }

  @Override public Query<T> setParameter(int index, Date parameter) {
    return (Query<T>) super.setParameter(index, parameter);
  }

  @Override public Query<T> setParameter(int index, Boolean parameter) {
    return (Query<T>) super.setParameter(index, parameter);
  }
}
