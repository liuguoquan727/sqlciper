...
/**
 * A thread-safe, unmodifiable list that reads entities once they are accessed from an underlying
 * database cursor. Make
 * sure to close the list once you are done with it. The lazy list can be cached or not. Cached lazy
 * lists store the
 * entities in memory to avoid loading entities more than once. Some features of the list are
 * limited to cached lists
 * (e.g. features that require the entire list). Cached lists close the cursor automatically once
 * you queried all
 * entities. However, to avoid leaked cursors, you should not rely on this behavior: if an exception
 * occurs before the
 * entire list is read, you should close the lazy list (and thus the underlying cursor) on your own
 * to be on the safe
 * side.
 *
 * @param <E> Entity type.
...
public class LazyList<E> implements List<E>, Closeable {
  protected class LazyIterator implements CloseableListIterator<E> {
    ...
    @Override
    /** FIXME: before hasPrevious(), next() must be called. */ public boolean hasPrevious() {
      return index > 0;
    }
    ...
    @Override
    /** FIXME: before previous(), next() must be called. */ public E previous() {
      if (index <= 0) {
  ...
  /**
   * Loads the remaining entities (if any) that were not loaded before. Applies to cached lazy lists
   * only.
   */
  ...
  @Override
  /** Closes the underlying cursor: do not try to get entities not loaded (using get) before. */ public void close() {
    cursor.close();
  }
  ...
  @Override public boolean addAll(Collection<? extends E> arg0) {
    throw new UnsupportedOperationException();
  }
  ...
  @Override public CloseableListIterator<E> listIterator() {
    return new LazyIterator(0, false);
  }
