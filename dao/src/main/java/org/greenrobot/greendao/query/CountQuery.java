...
public class CountQuery<T> extends AbstractQuery<T> {
  ...
  private final static class QueryData<T2> extends AbstractQueryData<T2, CountQuery<T2>> {
    ...
    @Override protected CountQuery<T2> createQuery() {
  ...
  private CountQuery(QueryData<T> queryData, AbstractDao<T, ?> dao, String sql,
  ...
  @Override public CountQuery<T> setParameter(int index, Object parameter) {
    return (CountQuery<T>) super.setParameter(index, parameter);
  }

  @Override public CountQuery<T> setParameter(int index, Date parameter) {
    return (CountQuery<T>) super.setParameter(index, parameter);
  }

  @Override public CountQuery<T> setParameter(int index, Boolean parameter) {
    return (CountQuery<T>) super.setParameter(index, parameter);
  }
}
