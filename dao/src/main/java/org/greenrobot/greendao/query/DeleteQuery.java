...
public class DeleteQuery<T> extends AbstractQuery<T> {
  private final static class QueryData<T2> extends AbstractQueryData<T2, DeleteQuery<T2>> {
    ...
    @Override protected DeleteQuery<T2> createQuery() {
  ...
  private DeleteQuery(QueryData<T> queryData, AbstractDao<T, ?> dao, String sql,
   ...
   * session/cache). Note that this
   * method may lead to stale entity objects in the session cache. Stale entities may be returned
   * when loaded by
   * their
  ...
  @Override public DeleteQuery<T> setParameter(int index, Object parameter) {
    return (DeleteQuery<T>) super.setParameter(index, parameter);
  }

  @Override public DeleteQuery<T> setParameter(int index, Date parameter) {
    return (DeleteQuery<T>) super.setParameter(index, parameter);
  }

  @Override public DeleteQuery<T> setParameter(int index, Boolean parameter) {
    return (DeleteQuery<T>) super.setParameter(index, parameter);
  }
}
