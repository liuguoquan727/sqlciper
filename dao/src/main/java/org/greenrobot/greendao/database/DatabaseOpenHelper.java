...
/**
 * SQLiteOpenHelper to allow working with greenDAO's {@link Database} abstraction to create and
...
public abstract class DatabaseOpenHelper extends SQLiteOpenHelper {
  ...
  /**
   * Like {@link #getWritableDatabase(String)} ()}, but returns a greenDAO abstraction of the
   * database.
   * The backing DB is an standard {@link SQLiteDatabase}.
   */
  public Database getWritableDb() {
    return wrap(getWritableDatabase("123"));
  }

  /**
   * Like {@link #getReadableDatabase(String)} ()}, but returns a greenDAO abstraction of the
   * database.
   * The backing DB is an standard {@link SQLiteDatabase}.
   */
  public Database getReadableDb() {
    return wrap(getReadableDatabase("123"));
  }
  ...
  /**
   * Delegates to {@link #onUpgrade(Database, int, int)}, which uses greenDAO's database
   * abstraction.
   */
  ...
  private class EncryptedHelper extends net.sqlcipher.database.SQLiteOpenHelper {
    ...
    @Override public void onCreate(net.sqlcipher.database.SQLiteDatabase db) {
      DatabaseOpenHelper.this.onCreate(wrap(db));
    }

    @Override public void onUpgrade(net.sqlcipher.database.SQLiteDatabase db, int oldVersion,
        int newVersion) {
      DatabaseOpenHelper.this.onUpgrade(wrap(db), oldVersion, newVersion);
    }

    @Override public void onOpen(net.sqlcipher.database.SQLiteDatabase db) {
