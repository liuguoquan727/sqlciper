 ...
 *
 * In case foreignKey is not specified, the following annotations can be applied together with
 *
 * @ToOne: - {@link Property} to specify backing column name
 ...
 */
@Retention(RetentionPolicy.SOURCE) @Target(ElementType.FIELD) public @interface ToOne {
  /**
   * Name of the property inside the current entity which holds the key of related entity.
   * If this parameter is absent, then an additional column is automatically created to hold the
   * key.
   */
